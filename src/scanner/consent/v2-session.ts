...
function providerVerificationCapability(provider: CmpAdapterProviderId, state: ConsentState, frameworks: ConsentFrameworkObservations, context?: unknown) {
  const base = assessRejectVerificationCapability({ providerState: state, frameworks });
  return provider === 'usercentrics' ? usercentricsVerificationCapability(base, context as Parameters<typeof usercentricsVerificationCapability>[1]) : base;
}
...
function actionPlanFor(provider: CmpAdapterProviderId, actions: AvailableAction[], banner: BannerState, context: unknown, action: ActionPlan['action'], category: ActionPlan['category'] = null, timings?: ConsentTimingValues) {
  ...
  const target = actionTargetFor(context, action, category);
  if ((provider === 'cookiebot' || provider === 'usercentrics') && candidate.availability === 'direct' && (!target?.target_ref || !target.attached || !target.visible || !target.enabled || !target.accessible_control || !target.frame_path?.length || target.shadow_mode === 'closed')) return null;
...
function telemetry(result: FinalConsentAuditResult, tracking: TrackingConsistencyResult, facts: BrowserConsentFacts, generic: GenericConsentDetectionResult, frameworkObservations: ConsentFrameworkObservations, controls: ConsentV2RolloutControls, provider: CmpAdapterProviderId | undefined, conflict: boolean, blocked: boolean, actionsEnabled: boolean, rolloutGateEligible: boolean, timeline: ConsentV2Timeline, geo: ConsentV2SessionInput['geo'], capture: PreparedConsentV2Session, actionDiagnostics?: { action_execution_eligible: boolean; requested_action: string | null; execution_strategy: string | null; activation_occurred: boolean; semantic_discovery_attempted: boolean; semantic_candidate_count: number; semantic_actionable_count: number; requested_action_target_resolved: boolean; target_resolution_reason: NonNullable<ConsentV2Telemetry['target_resolution_reason']>; verification_capability: VerificationCapability; verification_strong_families: string[]; verification_supporting_families: string[]; verification_contradicting_families: string[]; verification_independence_groups: string[]; verification_reason_codes: string[]; tcf_capability_observation: ConsentFrameworkObservations['tcf']; usercentrics_runtime_variant?: string }): ConsentV2Telemetry {
  ...
  return {
    ...
    ...(actionDiagnostics ? {
      ...
      runtime_variant: provider === 'cookiebot' ? (facts.observations.some((item) => item.selector === '#CybotCookiebotDialog' && item.visible) ? 'standard_dialog' : 'custom_template')
        : provider === 'usercentrics' ? actionDiagnostics.usercentrics_runtime_variant || 'runtime_without_actionable_banner' : null,
      ...(provider === 'usercentrics' ? { usercentrics_runtime_version: facts.usercentrics.runtime_version } : {}),
...
export async function runConsentV2Session(page: Page, input: ConsentV2SessionInput, prepared?: PreparedConsentV2Session): Promise<ConsentV2SessionOutput> {
  ...
  try {
    ...
    let actionBoundaryDiagnostics = selection.provider === 'usercentrics'
      ? targetResolutionDiagnostics(initialCapture.snapshot.semanticDiscovery, contexts.get('usercentrics'), 'reject_all', null)
      : { semantic_discovery_attempted: false, semantic_candidate_count: 0, semantic_actionable_count: 0, requested_action_target_resolved: false, target_resolution_reason: 'label_not_found' as NonNullable<ConsentV2Telemetry['target_resolution_reason']> };
    const requestedAction = selection.provider === 'usercentrics' || actions.some((item) => item.action === 'reject_all' && ['direct', 'api_only'].includes(item.availability)) ? 'reject_all'
    ...
    const telemetryResult = telemetry(result, tracking, before, generic, frameworkObservations, rollout, selection.provider, selection.conflict, false, actionExecutionEligible, providerActionGate, timeline, input.geo, capture, {
      ...
      ...(selection.provider === 'usercentrics' ? { usercentrics_runtime_variant: before.usercentrics.lifecycle.latest_view === 'SECOND_LAYER' ? 'uc_ui_second_layer'
        : before.usercentrics.lifecycle.latest_view === 'FIRST_LAYER' && before.globals.includes('UC_UI') ? 'uc_ui_first_layer'
          : before.usercentrics.shadow_mode === 'open' ? 'standard_open_shadow'
