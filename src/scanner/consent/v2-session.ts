...
export async function runConsentV2Session(page: Page, input: ConsentV2SessionInput, prepared?: PreparedConsentV2Session): Promise<ConsentV2SessionOutput> {
  ...
  try {
    ...
    const mechanisms = composeMechanisms(baseMechanisms, frameworkMechanisms(frameworks), googleConsentModeMechanism(gcm.result())); const contradiction = hasGcmContradiction(verification, gcm); const result = buildResult(input, mechanisms, banner, actions, initial, resulting, attempt ? [attempt] : [], verification, persistence, frameworks, gcm, requests, [...(mechanisms.length ? [] : [ConsentAuditCodes.NO_CMP_DETECTED]), ...verification.reason_codes, ...persistence.reason_codes, ...(contradiction ? [ConsentAuditCodes.STATE_CONTRADICTION] : [])]); const tracking = checkTrackingConsistency({ rejection_verification: verification, user_choice_at: timeline.user_choice_at, post_reject_observation_completed: persistence.status !== 'not_applicable', requests }); return { result, tracking, ledger, telemetry: telemetry(result, tracking, before, rollout, selection.provider, selection.candidates.filter((item) => item.high_confidence).length > 1, false, actionEnabled, timeline), google_consent_mode: gcm.result() };
