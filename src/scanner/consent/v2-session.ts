...
function providerVerificationCapability(provider: CmpAdapterProviderId, state: ConsentState, frameworks: ConsentFrameworkObservations) {
  const base = assessRejectVerificationCapability({ providerState: state, frameworks });
  return provider === 'usercentrics' ? usercentricsVerificationCapability(base) : base;
}
...
export async function runConsentV2Session(page: Page, input: ConsentV2SessionInput, prepared?: PreparedConsentV2Session): Promise<ConsentV2SessionOutput> {
  ...
  try {
    ...
    let verificationCapability = selection.provider
      ? providerVerificationCapability(selection.provider, provider.state, frameworkObservations)
