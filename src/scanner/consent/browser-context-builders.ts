...
export interface BrowserConsentFacts {
  ...
  generic: {
    surfaces: Array<{ id: string; surface_type: 'banner' | 'dialog' | 'drawer'; visible: boolean; privacy_or_cookie_semantics: boolean; intent: string; consent_management_topology: boolean }>;
...
export async function captureBrowserConsentFacts(page: Page): Promise<BrowserConsentFacts> {
  const facts = await page.evaluate(({ globals, selectors, consentCommandKey, providerEventKey }) => {
    ...
    const knownConsentAction = (name: string) => [
      'accept all', 'accept cookies', 'allow all', 'accept', 'alle akzeptieren', 'tout accepter',
      'reject all', 'decline all', 'deny all', 'reject', 'decline', 'alle ablehnen', 'continuer sans accepter',
      ...
      'manage cookie preferences', 'privacy preferences', 'customize', 'einstellungen', 'personnaliser'
