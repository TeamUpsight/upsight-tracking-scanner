...
export interface BrowserConsentFacts {
  ...
  generic: {
    ...
    controls: Array<{ surface_id: string; visible: boolean; enabled: boolean; actionable: boolean; accessible_name: string; location: 'main_frame' | 'shadow_dom'; shadow_depth: number }>;
  };
  usercentrics: {
    present: boolean;
    visible: boolean;
...
export async function captureBrowserConsentFacts(page: Page): Promise<BrowserConsentFacts> {
  const facts = await page.evaluate(({ globals, selectors, consentCommandKey, providerEventKey }) => {
    ...
    const genericSurfaceFacts = genericSurfaces.map((surface, index) => {
      ...
      return { id: `surface-${index}`, surface_type: surface.getAttribute('role') === 'dialog' || surface.getAttribute('aria-modal') === 'true' ? 'dialog' : 'banner', visible: visible(surface), privacy_or_cookie_semantics: privacyOrCookieSemantics, intent, consent_management_topology: consentManagementTopology, strong_presentation: surface.getAttribute('role') === 'dialog' || surface.getAttribute('aria-modal') === 'true' || style?.position === 'fixed' || style?.position === 'sticky', location: 'main_frame' as const, shadow_depth: 0 };
    });
  ...
  const usercentrics = await page.evaluate((rootSelector) => {
    ...
    if (!root) return { present: false, visible: false, shadow_mode: 'none' as const, controls: [] };
    ...
    if (!shadow) return { present: true, visible, shadow_mode: 'closed' as const, controls: [] };
    return {
      present: true,
      visible,
