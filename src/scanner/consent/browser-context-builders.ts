...
export async function buildProviderContexts(page: Page, facts: BrowserConsentFacts, framework: ConsentFrameworkObservations) {
  ...
  return new Map<CmpAdapterProviderId, unknown>([
    ...
    ['didomi', { ...common, window_globals: facts.globals, surfaces: DIDOMI_STANDARD_ROOTS.map((selector) => ({ selector, visible: Boolean(observation(facts, selector)?.visible) })), generic_surfaces: facts.generic.surfaces, controls: facts.didomi_controls.flatMap((item) => { const action = semanticActionForConsentLabel(item.accessible_name); return action === 'accept_all' || action === 'reject_all' || action === 'open_preferences' ? [{ id: item.id, semantic_action: action, origin: 'semantic_ui' as const, visible: item.visible, enabled: item.enabled, actionable: item.visible && item.enabled, within_confirmed_didomi_surface: true }] : []; }), action_targets: facts.didomi_controls.flatMap((item) => { const action = semanticActionForConsentLabel(item.accessible_name); return action === 'accept_all' || action === 'reject_all' || action === 'open_preferences' ? [{ action, category: null, target_ref: `didomi:${item.id}`, surface_type: 'banner' as const, frame_path: ['top'], shadow_mode: 'none' as const, accessible_control: true, attached: true, visible: item.visible, enabled: item.enabled } satisfies BrowserActionTarget] : []; }), public_methods: Array.isArray(facts.didomi?.public_methods) ? facts.didomi.public_methods : [], runtime: facts.didomi, provider_events: facts.provider_events, invoke_control: (id: string) => clickDidomiControl(page, id), invoke_public_method: (method: string) => page.evaluate((name) => { const api = (window as any).Didomi; const fn = name === 'preferences.show' ? api?.preferences?.show : api?.[name]; if (typeof fn !== 'function') return false; fn.call(name === 'preferences.show' ? api.preferences : api); return true; }, method).catch(() => false) }],
