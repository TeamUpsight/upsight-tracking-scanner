...
export async function buildProviderContexts(page: Page, facts: BrowserConsentFacts, framework: ConsentFrameworkObservations, semanticDiscovery?: ProviderSemanticDiscovery, geo?: 'USA' | 'EU' | 'UK') {
  ...
  return new Map<CmpAdapterProviderId, unknown>([
    ...
    ['usercentrics', {
      ...
      controls: (semanticDiscovery?.diagnostic.provider === 'usercentrics' ? semanticDiscovery.controls : []).map((item) => ({
        id: item.id, semantic_action: item.action, visible: item.visible, enabled: item.enabled,
        actionable: item.actionable, within_confirmed_usercentrics_surface: true, role: 'button' as const
      })),
      action_targets: (semanticDiscovery?.diagnostic.provider === 'usercentrics' ? semanticDiscovery.controls : []).map((item) => ({
        action: item.action, category: null, target_ref: item.id, surface_type: 'dialog' as const,
        frame_path: ['top'], shadow_mode: 'open' as const, accessible_control: true,
        attached: true, visible: item.visible, enabled: item.enabled
      } satisfies BrowserActionTarget)),
      legacy_globals: facts.globals,
      invoke_control: (id: string) => id.startsWith('usercentrics-semantic:')
        ? semanticDiscovery?.invoke(id) || Promise.resolve(false) : Promise.resolve(false)
    }],
