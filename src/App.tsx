            ...
            <div className="min-w-0 space-y-4">
              {selected ? <>
                <section className="rounded-2xl border border-neutral-border bg-bg-card p-5 shadow-sm">
                  ...
                  {selected.evidence_bundle?.access && (() => { const access = selected.evidence_bundle.access; return <div className="mt-4 rounded-xl border border-neutral-border-muted bg-[#10141c] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500">Access & runtime</div><div className="mt-1 text-xs font-semibold text-white">{accessStateLabel(selected)}</div></div><StatusBadge value={access.valid_storefront === true ? 'valid storefront reached' : access.valid_storefront === false ? 'storefront not reached' : 'inconclusive'} /></div><div className="mt-3 grid gap-2 text-[10px] sm:grid-cols-2 xl:grid-cols-4"><div><span className="text-slate-500">Initial provider</span><p className="mt-0.5 text-slate-300">{proxyLabel(access.initial_provider)}</p></div><div><span className="text-slate-500">Final provider</span><p className="mt-0.5 text-slate-300">{proxyLabel(access.final_provider, access.initial_provider === 'decodo' && access.final_provider === 'decodo' && access.access_attempt_count > 1)}</p></div><div><span className="text-slate-500">Retries</span><p className="mt-0.5 text-slate-300">{Math.max(0, access.access_attempt_count - 1)}</p></div><div><span className="text-slate-500">Access duration</span><p className="mt-0.5 text-slate-300">{formatDuration(access.time_to_valid_storefront_ms)}</p></div><div><span className="text-slate-500">Browserless fallback</span><p className="mt-0.5"><StatusBadge value={access.proxy_fallback_used ? access.proxy_fallback_recovered ? 'recovered' : 'used' : 'not used'} /></p></div><div><span className="text-slate-500">Challenge</span><p className="mt-0.5"><StatusBadge value={access.challenge_detected ? access.challenge_type || 'unknown challenge' : 'not detected'} /></p></div><div><span className="text-slate-500">Solver</span><p className="mt-0.5"><StatusBadge value={access.challenge_solver_used ? access.challenge_solver_result : 'not used'} /></p></div><div><span className="text-slate-500">HTTP</span><p className="mt-0.5 text-slate-300">{access.http_status ?? '—'}</p></div></div></div>; })()}
