...
function proxyLabel(provider: 'decodo' | 'browserless_residential' | undefined, retry = false) {
            ...
            <div className="min-w-0 space-y-4">
              {selected ? <>
                ...
                <section className="rounded-2xl border border-neutral-border bg-bg-card p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-bold text-white">Evidence Summary</h3><p className="mt-1 text-[11px] text-slate-400">Compact operational context and machine-readable reason codes.</p></div>{selected.evidence_bundle?.product.pdp_url && <a href={selected.evidence_bundle.product.pdp_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] text-primary hover:underline">Open Tested PDP<ExternalLink className="h-3 w-3" /></a>}</div>
                  ...
                  {selected.evidence_bundle && <details className="mt-4 rounded-xl border border-neutral-border-muted bg-[#10141c] p-3"><summary className="cursor-pointer text-[10px] font-bold uppercase tracking-wider text-slate-400">Diagnostics & health</summary><div className="mt-3 grid gap-3 text-[11px] text-slate-300 sm:grid-cols-2"><div><div className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Build</div><p className="mt-1.5">Scanner {selected.evidence_bundle.scanner_version} · Rule pack {selected.evidence_bundle.rule_pack_version}</p><p className="mt-1 text-slate-500">Build {selected.evidence_bundle.build_commit || 'local'} · {selected.evidence_bundle.build_timestamp}</p></div>{selected.evidence_bundle.mode === 'diagnostic' && <div><div className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Proxy path</div><p className="mt-1.5">{proxyLabel(selected.evidence_bundle.runtime.proxy_initial_provider)} → {proxyLabel(selected.evidence_bundle.runtime.proxy_final_provider, selected.evidence_bundle.runtime.proxy_initial_provider === 'decodo' && selected.evidence_bundle.runtime.proxy_final_provider === 'decodo' && (selected.evidence_bundle.runtime.proxy_retry_count || 0) > 0)}</p><p className="mt-1 text-slate-500">{selected.evidence_bundle.runtime.proxy_fallback_used ? 'Browserless fallback used' : 'No Browserless fallback'} · {selected.evidence_bundle.runtime.proxy_fallback_recovered || selected.evidence_bundle.runtime.proxy_retry_recovered ? 'recovered' : 'not recovered'}</p>{(selected.evidence_bundle.runtime.proxy_attempts || []).length > 0 && <ul className="mt-2 space-y-1 text-[10px] text-slate-400">{selected.evidence_bundle.runtime.proxy_attempts!.map((attempt) => <li key={`${attempt.provider}-${attempt.attempt}`}>{proxyLabel(attempt.provider, attempt.provider === 'decodo' && attempt.attempt > 0)} · attempt {attempt.attempt + 1} · {attempt.target_result || attempt.failure_reason || 'pending'}</li>)}</ul>}</div>}</div></details>}
