...
type AuditFilter = 'all' | 'review' | 'failed' | 'timeout' | 'proxy' | 'access' | 'runtime' | 'active';
...
function terminalFailure(audit: StorefrontAudit) {
  ...
  const reasonCode = typeof terminal?.reason_code === 'string' ? terminal.reason_code :
    typeof terminal?.error_category === 'string' ? terminal.error_category : audit.error_category;
  return {
    phase: terminal?.step ? formatLabel(terminal.step) : 'Terminal finalization',
    reasonCode,
...
export default function App() {
  ...
  const filteredScans = useMemo(() => {
    ...
    return scans.filter((scan) => {
      ...
      const matchesFilter = auditFilter === 'all' ||
        ...
        ['failed', 'timeout', 'proxy', 'access', 'runtime'].includes(auditFilter) && failureMatches(scan, auditFilter) ||
  ...
  const bulkRerunSelected = async () => {
    ...
    if (!ids.length || !window.confirm(`Re-run ${ids.length} selected audit${ids.length === 1 ? '' : 's'} using each audit's original geo, mode, and group label?`)) return;
    setBusy(true); setError(null);
    try {
            ...
            <section className="overflow-hidden rounded-2xl border border-neutral-border bg-bg-card shadow-sm lg:sticky lg:top-24">
              <div className="border-b border-neutral-border p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex gap-1 overflow-x-auto">{([['all', 'All'], ['review', 'Needs review'], ['failed', 'Failed'], ['timeout', 'Timeout'], ['proxy', 'Proxy'], ['access', 'Access'], ['runtime', 'Runtime'], ['active', 'Active']] as const).map(([key, label]) => <button type="button" key={key} onClick={() => setAuditFilter(key)} className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-semibold transition ${auditFilter === key ? 'bg-primary/15 text-primary' : 'text-slate-500 hover:bg-white/[0.04] hover:text-white'}`}>{label}</button>)}</div>
                ...
                {selectedAuditIds.size > 0 && <div className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-primary/25 bg-primary/[0.06] px-3 py-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <ExplainedAction help="Create a new audit for every selected row, preserving each audit's geo, scan mode, and group label." type="button" disabled={busy} onClick={() => void bulkRerunSelected()} className="border-primary/35 px-2.5 text-primary hover:bg-primary/10"><RotateCcw className="h-3.5 w-3.5" />Re-run</ExplainedAction>
            ...
            <div className="min-w-0 space-y-4">
              {selected ? <>
                <section className="rounded-2xl border border-neutral-border bg-bg-card p-5 shadow-sm">
                  ...
                  {selected.scan_status === 'failed' && (() => { const failure = terminalFailure(selected); return <div className="mt-4 rounded-xl border border-rose-800/50 bg-rose-950/20 px-3 py-3" role="status"><div className="text-[9px] font-bold uppercase tracking-[0.14em] text-rose-300">Scan failed</div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs"><span className="font-semibold text-white">Terminal phase: {failure.phase}</span><span className="font-mono text-[10px] text-rose-200" title={failure.reasonCode}>{formatLabel(failure.reasonCode)}</span></div>{failure.productPhase && <p className="mt-1.5 text-[11px] text-rose-100">Homepage evidence completed; PDP/product module incomplete.</p>}</div>; })()}
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    ...
                    <ExplainedAction type="button" help="Creates a new diagnostic audit for this domain and geo. It uses the same detector rules, retains more bounded evidence and screenshots, and does not enable challenge escalation." onClick={async () => { const result = await selectedAction('diagnostic-rerun'); if (result?.audit_id) setSelectedId(result.audit_id); }} className="border-primary/40 text-primary hover:bg-primary/10"><Bug className="h-3.5 w-3.5" />Re-run diagnostic</ExplainedAction>
                    <ExplainedAction type="button" help="Creates a manual diagnostic audit with the configured CAPTCHA or challenge retry enabled. Use this for high-value blocked sites, not routine bulk scanning." onClick={async () => { const result = await selectedAction('difficult-site-rerun'); if (result?.audit_id) setSelectedId(result.audit_id); }} className="border-violet-700/70 text-violet-300 hover:bg-violet-950/30"><FlaskConical className="h-3.5 w-3.5" />Re-run difficult site</ExplainedAction>
                ...
                <section className="rounded-2xl border border-neutral-border bg-bg-card p-4 shadow-sm"><div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="text-sm font-bold text-white">Audit Findings</h3><p className="mt-1 text-[11px] text-slate-400">Evidence-based statuses. “Not tested” means the module did not run; “inconclusive” means it ran without enough evidence.</p></div><span className="text-[10px] uppercase tracking-wider text-slate-400">{selected.overall_confidence || '—'} confidence</span></div><div className="grid gap-3 xl:grid-cols-2">
                  ...
                  <Finding title="GA4" description={(selected.site_ga4_measurement_ids || []).length ? `Measurement ID${selected.site_ga4_measurement_ids!.length > 1 ? 's' : ''}: ${selected.site_ga4_measurement_ids!.join(', ')}` : 'No GA4 measurement ID retained.'} value={boolStatus(selected.site_ga4_detected)} confidence={selected.finding_confidence?.ga4} />
                  <Finding title="GA4 Ecommerce View Item" description={selected.pdp_url_tested ? <a href={selected.pdp_url_tested} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 break-all text-primary hover:underline">{selected.pdp_url_tested}<ExternalLink className="h-3 w-3 shrink-0" /></a> : 'No PDP URL was successfully tested.'} value={selected.product_payload_status} confidence={selected.finding_confidence?.product} />
                ...
                <section className="rounded-2xl border border-neutral-border bg-bg-card p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-bold text-white">Evidence Summary</h3><p className="mt-1 text-[11px] text-slate-400">Compact operational context and machine-readable reason codes.</p></div>{selected.evidence_bundle?.product.pdp_url && <a href={selected.evidence_bundle.product.pdp_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] text-primary hover:underline">Open Tested PDP<ExternalLink className="h-3 w-3" /></a>}</div>
                  <div className="mt-4 grid gap-2 sm:grid-cols-3 xl:grid-cols-6"><MetricCard label="Requests" value={selected.evidence_bundle?.network.total_requests ?? '—'} /><MetricCard label="Duration" value={formatDuration(selected.evidence_bundle?.runtime.total_duration_ms)} /><MetricCard label="Evidence" value={selected.evidence_bundle ? `${Math.round(selected.evidence_bundle.runtime.evidence_size_bytes / 1024)} KB` : '—'} /><MetricCard label="Proxy retries" value={selected.evidence_bundle?.runtime.proxy_retry_count ?? '—'} /><MetricCard label="Proxy country" value={selected.evidence_bundle?.runtime.proxy_country || 'unverified'} /><MetricCard label="Challenge" value={selected.evidence_bundle?.page.challenge_cleared ? 'cleared' : selected.evidence_bundle?.page.bot_provider ? 'not cleared' : 'none seen'} /></div>
