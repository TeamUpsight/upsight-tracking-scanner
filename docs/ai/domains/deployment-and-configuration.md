...
`dist/` and `node_modules/` are generated/installed content. The archive has no CI, Dockerfile, Wrangler file, or infrastructure-as-code manifest. Build provenance includes `scanner_version`, `scanner_execution_mode`, `build_commit`, `build_dirty`, `build_timestamp`, and `certification_eligible`; it is exposed through health, `scan_started`, Evidence Bundles, and debug exports. A clean compiled artifact is required for certifiable live provider runs. Build after the local commit with `npm run build`, then launch `node dist/server.cjs`; verify `/api/health` reports `compiled_bundle`, the current commit, `build_dirty=false`, and `certification_eligible=true` before submitting a live diagnostic audit. `npm run dev` uses TSX and is never a certification entrypoint. The production build also emits `dist/browser-facts-smoke.cjs` solely for the deterministic compiled browser smoke test.
