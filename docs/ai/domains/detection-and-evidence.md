...
The production Usercentrics bridge currently has no validated semantic runtime decision reader or independent semantic Reject event. One TCF listener alone cannot satisfy the Usercentrics pre-action verification gate, and runtime state and its category projection count as one `usercentrics_runtime` independence group. Storage descriptors and lifecycle events are supporting observations only. Persistence reload remains gated on a verified Reject, so the current Congstar shape does not reload or claim a persisted rejection. Bounded action diagnostics report target resolution and verification reasons without retaining selectors, raw shadow markup, or event payloads.
