...
Usercentrics identity uses exact provider loaders, `UC_UI`, or a known root; German consent labels alone are never provider identity. Lifecycle observations (`UC_UI_INITIALIZED`, `UC_UI_VIEW_CHANGED`, `CMP_SHOWN`) establish readiness or visibility, never a consent decision. The browser bridge observes generic and delayed open-shadow controls for diagnostics, but only an exact, visible, enabled control inside the known open `aside#usercentrics-cmp-ui` shadow root can become an executable target. A duplicate owner is ambiguous. The Congstar-shaped generic shadow host lacks a verified Usercentrics host signature and remains observation-only.
...
The only certified structural marker in repository fixtures is the exact `aside#usercentrics-cmp-ui` root with an open shadow tree. The census names this `usercentrics_named_root`, but a root without an open shadow remains unsupported for actions. A full `usercentrics-` identifier prefix or `data-usercentrics` attribute name is recorded only as `provider_prefix_candidate_present` within a confirmed surface. This prefix is derived from the existing root's provider name and is a diagnostic hint, not a validated main-frame selector or ownership proof. Generic `uc`, `cookie`, `consent`, `privacy`, `cmp`, `banner`, and `modal` tokens cannot establish Usercentrics ownership. No second main-frame topology has been certified; `main_frame_provider_owned` is reserved for a future separately proven marker and negative fixture set.

The diagnostic projection also retains the v2 reader's existing normalized pre-choice decision and explicit-decision boolean (or `unknown` when unreadable). This does not change v2 semantics, action eligibility, provenance requirements, or provider API interaction policy. A further compiled, clean, EU Congstar observation-only audit is needed to see whether the bounded confirmed surface contains a stable provider-specific marker. If the census finds no qualifying surface or marker, target ownership remains unproven and direct Reject remains observation-only.
