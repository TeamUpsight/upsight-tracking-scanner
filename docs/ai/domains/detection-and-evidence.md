...
The optional USA diagnostic `GPC_EXPERIMENT_ENABLED` path uses two fresh, service-worker-blocked contexts. In Browserless, each arm now has a separate browser process/session built from the same canonical route, proxy-session URL, locale, and launch options. Only OFF adds a Chromium launch flag disabling its GPC force/test features; ON retains the existing launch. Local fixtures retain the same-browser two-context path. OFF no longer tries to remove the observed Browserless-session GPC header through Playwright routing. The per-arm context stage permits up to 12 seconds for the added Browserless connection, within the unchanged arm and global budgets. CDP sent-header events still record bounded first-party and top-level `Sec-GPC` states, and the DOM value must also match; configuration alone never validates transport. Session comparisons require matching browser versions, configured identity, locale/timezone, and independently verified USA egress fingerprints. Separate Browserless sessions may yield different egress identities, in which case the result remains `identity_unmatched`. The pair reuses passive Consent UI, US privacy, GPP, Access, and normalized Consent measurement observers; it never clicks, scans Product/Server-side, or contributes to replay. Invalid transport, identity, access, or budget yields an inconclusive technical result. No outcome asserts legal compliance or that GPC was honored. Audit 513 proves `Sec-GPC: 1` appeared after Playwright's OFF route override and by CDP's sent-header observation, but does not distinguish a Browserless launch preference from Chromium or stealth configuration. The new Browserless OFF launch still requires one authorized live sent-header validation; local Chromium tests cannot prove the hosted Browserless configuration.
